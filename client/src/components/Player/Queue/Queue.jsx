      ...
      >
        <AppBar sx={{ position: 'relative', backgroundColor: '#000A1F' }}>
          <Toolbar>
            ...
            <Typography sx={{ ml: 2, flex: 1, fontFamily: 'Inter, sans-serif', fontWeight: '600' }} variant="h4" component="div">
              Queue
            </Typography>
          </Toolbar>
        </AppBar>
          ...
          {tracks?.map((t, i) =>
            ...
            <ListItem className={s.queueTrack} sx={{color: 'white', cursor: 'pointer'}} key={`${t}_${Math.random()}`} button onClick={() => handleTrack(i)}>
              <div className={s.queueItem}>
                ...
                <ListItemText className={s.trackInfo} sx={{color: 'white'}} primary={t.title} secondary='Artist' />
                { !isPlaying
