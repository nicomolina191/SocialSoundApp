...
const AdminGraphs = () => {
  ...
  return (
    <Box className={style.backgroundContainer}>
      ...
      {typeof postDataGraphs === "string" && <h1 style={{color: "white"}}>{postDataGraphs}</h1>}
      {typeof postDataGraphs !== "string" && <AreaComponent dates={postDataGraphs} step={activeStep}/>}
      </Box>
<Box>
  {typeof postDataGraphs !== "string" && <MobileStepper 
  sx={{ backgroundColor:"var(--main-page-color)", color:"white", minWidth: 200, flexGrow: 1, width: "100%"}}
      variant="dots"
      steps={maxSteps}
      position="static"
      activeStep={activeStep}
      nextButton={
        <Button sx={{color:"var(--second-page-color)"}} size="small" onClick={handleNext} disabled={ activeStep >= maxSteps - 1}>
          Next
          <KeyboardArrowRight />
        </Button>
      }
      backButton={
        <Button sx={{color:"var(--second-page-color)"}} size="small" onClick={handleBack} disabled={activeStep <= 0}>
            <KeyboardArrowLeft />
