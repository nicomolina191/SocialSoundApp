...
const Input = () => {
  ...
  const handleChange = async (e) => {
    e.target.name === 'message' && setMessageData({...messageData, message: e.target.value})
    e.target.name === 'img' && setMessageData({...messageData, img: await uploadFile(e.target.files[0])})
  }
  ...
  const handleSend = async (e) => {
    ...
    if (image) {
      ...
      content && await updateDoc(doc(db, "chats", chatId), {
        messages: arrayUnion({
          id: uuid(),
          text:content,
          senderId: currentUser.idgoogle,
    ...
    content && await updateDoc(doc(db, "userConversations", currentUser.idgoogle), {
      [chatId + ".lastMessage"]: {
        text:content,
      },
    ...
    content && await updateDoc(doc(db, "userConversations", destination.idgoogle), {
      [chatId + ".lastMessage"]: {
        text:content,
      },
      [chatId + ".date"]: serverTimestamp(),
    ...
    <div className={s.input}>
      ...
      {!imgLoading
        ...
        <input
          ...
          />
          {messageData.img ? <img className={s.imagePreview} src={messageData.img} alt="not found" /> : ''}
        </div>   
      <div className={s.sendBtn}>
        <button className={s.inputSubmit} type="submit"><svg width="15" height="15" viewBox="0 0 10 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M1 0.999999L9 9L0.999999 17" stroke="white" strokeWidth="2.4" strokeLinecap="round"/>
          </svg>
        </button> 
      </div>
      </form>
